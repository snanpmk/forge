...
export default function Dashboard() {
  ...
  const { habits, prayers, goals, brainDumpCount, finance, tasks, budgets } = data;
  ...
  return (
    <div className="pb-10 space-y-8 animate-fade-in relative z-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {/* Column 1: Habits (Full width on tablet, Left on Desktop) */}
        <div className="md:col-span-2 lg:col-span-1 space-y-6">
          <section className="glass-panel p-6 rounded-3xl relative overflow-hidden border border-white/60 h-full">
                ...
                habits.map((habit, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-4 overflow-hidden">
                      <button className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${habit.completedToday ? 'bg-wellness-green text-green-700 shadow-sm scale-110' : 'bg-white border border-gray-100 text-gray-300 group-hover:border-green-200'}`}>
                      ...
                      <div className="min-w-0">
                          <div className={`text-lg font-medium transition-colors truncate ${habit.completedToday ? 'text-muted line-through decoration-gray-300' : 'text-primary'}`}>
                          {habit.title}
                          </div>
                      </div>
                    </div>
        ...
        {/* Column 3: Finance & Budget */}
        <div className="space-y-6">
           {/* Budget Snapshot */}
           {budgets && budgets.length > 0 && (
            <div className="card bg-white border-white/50">
              <h3 className="font-bold text-primary mb-5 flex items-center gap-2 text-base">
                <DollarSign size={18} className="text-primary" /> Budget Tracker
              </h3>
              <div className="space-y-4">
                 {budgets.map(budget => (
                    <div key={budget._id} className="bg-gray-50/50 p-3 rounded-2xl border border-gray-100 hover:bg-white hover:shadow-sm transition-all">
                      <div className="flex justify-between text-xs mb-2">
                        <span className="font-bold text-primary">{budget.category}</span>
                        <span className="text-muted">
                            <span className={clsx("font-bold", budget.percentage >= 100 ? "text-red-500" : budget.percentage >= 80 ? "text-orange-400" : "text-emerald-500")}>
                      ...
                      <div className="w-full bg-gray-100 rounded-full h-2 mb-1 overflow-hidden">
                        <div 
                          className={clsx("h-full rounded-full transition-all duration-1000 ease-out", budget.percentage >= 100 ? "bg-red-500" : budget.percentage >= 80 ? "bg-orange-400" : "bg-emerald-500")}
